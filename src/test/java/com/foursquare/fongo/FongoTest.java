...
public class FongoTest {
     ...
     * 3. fetch referring entity
     * 4. update referred entity
     * 5. assert that initial object got update
     *
     * @throws Exception
     */
    @Test
    public void testUpdateDBRefs() throws Exception {

        Fongo fongo = newFongo();
        DB fongoDB = fongo.getDB("testDB");

//      1. save referred object
        FongoDBCollection refferredCollection = (FongoDBCollection) fongoDB.getCollection("referredCollection");
        DBObject referredObject = new BasicDBObject();
        referredObject.put("_id","test_ref_id");
        referredObject.put("changingProperty","initial");
        refferredCollection.save(referredObject);

//      2. insert object with ref into collection
        DBObject objectWithRef = new BasicDBObject();
        objectWithRef.put("_id","testId1");
        DBRefBase refObject =  new DBRefBase(fongoDB,refferredCollection.getName(),referredObject.get("_id"));
        BasicDBList listStub = new BasicDBList();
        listStub.add(refObject);
        objectWithRef.put("testList",listStub);
        FongoDBCollection referringCollection = (FongoDBCollection) fongoDB.getCollection("referringCollection");
        referringCollection.save(objectWithRef);

//      3. fetch referring entity
        DBObject toAssert = referringCollection.findOne();
        DBRefBase ref = (DBRefBase) ((BasicDBList)toAssert.get("testList")).get(0);
        assertThat(ref.fetch().get("changingProperty").toString(),is("initial"));

//      4. update referred entity
        referredObject = new BasicDBObject();
        referredObject.put("_id","test_ref_id");
...
//      5. assert that initial object got update
        ...
        ref = (DBRefBase) ((BasicDBList)toAssert.get("testList")).get(0);
